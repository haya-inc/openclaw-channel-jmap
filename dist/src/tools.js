...
function readEmailIds(params) {
    const raw = params.emailIds;
    if (!Array.isArray(raw)) {
        throw new Error("emailIds must be an array");
    }
    const ids = raw.map(optionalString).filter((id) => Boolean(id));
    if (ids.length === 0) {
...
function emailResult(email, options) {
...
function parsedEmailResult(sourceBlobId, email, maxBodyBytes) {
    const normalized = {
        ...email,
...
const fromEmailParam = Type.Optional(Type.String({
    description: "Concrete From address. Required for wildcard identities such as *@example.com.",
}));
const previewTokenParam = Type.String({
    description: "Exact token returned by the most recent jmap_mail_draft_preview for this draft and identity.",
});
const draftRecipientsParam = (description) => Type.Optional(Type.Array(Type.String(), {
    maxItems: 100,
    description,
}));
const draftSubjectParam = Type.Optional(Type.String({
    maxLength: 998,
    description: "Draft subject, limited to 998 UTF-8 bytes by the client.",
}));
const draftTextParam = Type.Optional(Type.String({
    maxLength: 1_000_000,
    description: "Plain-text draft body, limited to 1 MB of UTF-8.",
}));
const draftAttachmentsParam = Type.Optional(Type.Array(Type.Object({
    blobId: Type.String({ minLength: 1, description: "Existing JMAP blob id." }),
    type: Type.Optional(Type.String({ description: "MIME media type. Default application/octet-stream." })),
    name: Type.Optional(Type.String({ maxLength: 998 })),
    disposition: Type.Optional(Type.String({ description: "Usually attachment or inline." })),
    cid: Type.Optional(Type.String()),
    language: Type.Optional(Type.Array(Type.String(), { maxItems: 20 })),
    location: Type.Optional(Type.String()),
}, { additionalProperties: false }), {
    maxItems: 50,
    description: "Complete attachment list using existing blob ids. On update, omit to preserve or pass [] to remove all.",
}));
function optionalDraftAttachments(params) {
...
async function runAuditedJmapTool(toolName, params, operation) {
    ...
    try {
        ...
        logger.warn(`tool invocation failed name=${toolName} durationMs=${completedAt - startedAt} errorType=${error instanceof Error ? error.name : typeof error}`);
        throw error;
    }
}
    ...
    const tools = [
        {
            ...
            description: "List readable JMAP mailboxes with roles, message counts, and mailbox rights. This has no side effect.",
        ...
        },
        {
            ...
            description: "Search a selected mailbox or all readable mail using JMAP filters. Returns message metadata, pagination, and previews, not full bodies.",
            parameters: Type.Object({
                accountId: accountIdParam,
                mailbox: Type.Optional(Type.String({
                    description: "Mailbox id, role, or exact name. Use 'all' for every readable mailbox. Default inbox.",
                })),
                ...
                after: Type.Optional(Type.String({ description: "RFC 3339 lower time bound, for example 2026-07-01T00:00:00Z." })),
                before: Type.Optional(Type.String({ description: "RFC 3339 upper time bound, for example 2026-08-01T00:00:00Z." })),
                unread: Type.Optional(Type.Boolean({ description: "True for unread, false for read." })),
                hasAttachment: Type.Optional(Type.Boolean({ description: "Filter by whether the email has an attachment." })),
                minSize: Type.Optional(Type.Integer({ minimum: 0, description: "Minimum email size in bytes." })),
                maxSize: Type.Optional(Type.Integer({ minimum: 0, description: "Exclusive maximum email size in bytes." })),
                hasKeyword: Type.Optional(Type.String({ description: "Require a JMAP keyword, for example $answered." })),
                notKeyword: Type.Optional(Type.String({ description: "Exclude a JMAP keyword, for example $draft." })),
                collapseThreads: Type.Optional(Type.Boolean({ description: "Return at most one email per thread. Default false." })),
                position: Type.Optional(Type.Integer({ minimum: 0, description: "Zero-based result position. Default 0." })),
                limit: Type.Optional(Type.Integer({ minimum: 1, maximum: 100, description: "Maximum results. Default 20." })),
            }, { additionalProperties: false }),
            execute: async (_toolCallId, rawParams) => {
                const params = rawParams;
                return runAuditedJmapTool("jmap_mail_search", params, async () => {
                    ...
                    const page = await client.searchEmailPage({
                        ...
                        collapseThreads: typeof params.collapseThreads === "boolean" ? params.collapseThreads : undefined,
                    ...
                    return jsonResult({
                        ...
                        emails: page.emails.map((email) => emailResult(email, {
                            includeBody: false,
        ...
        },
        {
            ...
            description: "Read one bounded page of created, updated, and destroyed ids since a prior JMAP state token. Persist newState and continue while hasMoreChanges is true.",
            parameters: Type.Object({
                accountId: accountIdParam,
                dataType: Type.Union([
                ...
                maxChanges: Type.Optional(Type.Integer({
                    minimum: 1,
                    maximum: 1_000,
            ...
            execute: async (_toolCallId, rawParams) => {
                ...
                return runAuditedJmapTool("jmap_mail_changes", params, async () => {
                    const dataType = requiredString(params, "dataType");
                    const { account, client } = await resolveClient(optionalString(params.accountId));
                    const changes = await client.getChanges(dataType, requiredString(params, "sinceState"), typeof params.maxChanges === "number" ? params.maxChanges : undefined);
                    return jsonResult({
                        accountId: account.accountId,
                        notice: "Persist newState only after downstream processing succeeds. Continue from newState while hasMoreChanges is true.",
                        ...changes,
                    });
                });
            },
        },
        {
            ...
            description: "Parse uploaded or attached RFC 5322 message blobs without importing them into a mailbox. Bodies are bounded and treated as untrusted content.",
            parameters: Type.Object({
                accountId: accountIdParam,
                ...
                maxBodyBytes: Type.Optional(Type.Integer({
                    minimum: 1,
                    maximum: 1_000_000,
            ...
            execute: async (_toolCallId, rawParams) => {
                ...
                return runAuditedJmapTool("jmap_mail_parse", params, async () => {
                    ...
                    const { account, client } = await resolveClient(optionalString(params.accountId));
                    const maxBodyBytes = typeof params.maxBodyBytes === "number"
                        ? params.maxBodyBytes
                    ...
                    return jsonResult({
                        safetyNotice: SAFETY_NOTICE,
                        accountId: account.accountId,
                        parsed: Object.entries(result.parsed).map(([blobId, email]) => parsedEmailResult(blobId, email, maxBodyBytes)),
        ...
        },
        {
            ...
            description: "Upload at most 5 MiB to the configured mail account as an unreferenced JMAP blob. This does not create, import, or send an email, but it transfers content to the server.",
            parameters: Type.Object({
                accountId: accountIdParam,
            ...
            execute: async (_toolCallId, rawParams) => {
                ...
                return runAuditedJmapTool("jmap_mail_blob_upload", params, async () => {
                    ...
                    return jsonResult({
                        ...
                        notice: "Blob uploaded but not referenced. Use its blobId in a draft attachment or an explicit import before the server expires it.",
                        uploaded,
                    });
                });
            },
        },
        {
            ...
            description: "Import existing RFC 5322 blobs into one mailbox without sending them. This creates mailbox objects and requires explicit confirmation.",
            parameters: Type.Object({
                accountId: accountIdParam,
                blobIds: Type.Array(Type.String(), {
                    minItems: 1,
                ...
                ifInState: Type.Optional(Type.String({ description: "Optional Email state token for optimistic concurrency." })),
                confirm: Type.Literal(true, {
        ...
        },
        {
            ...
            description: "Copy emails from the configured JMAP mail account to another accessible account in the same JMAP Session. Originals are never destroyed.",
            parameters: Type.Object({
                accountId: accountIdParam,
                emailIds: Type.Array(Type.String(), {
                    minItems: 1,
                ...
                ifFromInState: Type.Optional(Type.String({ description: "Optional source Email state token." })),
                ifInState: Type.Optional(Type.String({ description: "Optional destination Email state token." })),
                confirm: Type.Literal(true, {
            ...
            execute: async (_toolCallId, rawParams) => {
                ...
                return runAuditedJmapTool("jmap_mail_copy", params, async () => {
                    ...
                    const result = await client.copyEmails({
                        ...
                        destinationMailboxIds: optionalStringArray(params, "destinationMailboxIds") ?? [],
                        keywords: optionalStringArray(params, "keywords"),
        ...
        },
        {
            ...
            parameters: Type.Object({
                ...
                markRead: Type.Optional(Type.Boolean({ description: "Mark the email as read after fetching it. Default false." })),
            }, { additionalProperties: false }),
            execute: async (_toolCallId, rawParams) => {
                const params = rawParams;
        ...
        },
        {
            ...
            description: "Read a bounded page of a JMAP thread in chronological order, newest page first. Defaults to the latest 20 emails.",
            parameters: Type.Object({
                accountId: accountIdParam,
                ...
                offset: Type.Optional(Type.Integer({
                    minimum: 0,
                    description: "Number of newest emails to skip when reading an older page. Default 0.",
                })),
            }, { additionalProperties: false }),
            execute: async (_toolCallId, rawParams) => {
                const params = rawParams;
        ...
        },
        {
            ...
            parameters: Type.Object({
                ...
                applyIdentitySignature: Type.Optional(Type.Boolean({
                    description: "Append the selected identity's plain-text signature. Default false.",
                })),
            }, { additionalProperties: false }),
            execute: async (_toolCallId, rawParams) => {
                const params = rawParams;
                return runAuditedJmapTool("jmap_mail_draft_create", params, async () => {
                    ...
                    const draft = await client.createDraft({
                        ...
                        applyIdentitySignature: typeof params.applyIdentitySignature === "boolean"
        ...
        },
        {
            ...
            description: "Read an exact, bounded preview of a draft and return a content-bound token. Preview again after every edit; the token is required to submit or discard.",
            parameters: Type.Object({
                accountId: accountIdParam,
            ...
            execute: async (_toolCallId, rawParams) => {
                ...
                return runAuditedJmapTool("jmap_mail_draft_preview", params, async () => {
                    ...
                    return jsonResult({
                        ...
                        submitted: false,
                        sent: false,
                        next: "Review every field. To edit, pass this previewToken to jmap_mail_draft_update, then preview the replacement. To send or discard, pass the current token to the matching explicit action.",
                        preview,
                    });
                });
            },
        },
        {
            ...
            description: "Replace a previously previewed immutable JMAP draft with an edited draft. The original is removed only after the replacement is created. Preview the new Email id before any final action.",
            parameters: Type.Object({
                accountId: accountIdParam,
                ...
                attachments: draftAttachmentsParam,
                applyIdentitySignature: Type.Optional(Type.Boolean({
                    description: "Append the selected identity's plain-text signature when text is replaced.",
                })),
            }, { additionalProperties: false }),
            execute: async (_toolCallId, rawParams) => {
                const params = rawParams;
                return runAuditedJmapTool("jmap_mail_draft_update", params, async () => {
                    ...
                    return jsonResult({
                        ...
                        submitted: false,
                        sent: false,
                        notice: "Replacement saved. The prior preview token is now invalid; preview the replacement Email id before submitting or discarding.",
                        replacement,
                    });
                });
            },
        },
        {
            ...
            description: "Permanently discard the exact draft content that was most recently previewed. This is not submission cancellation and requires explicit confirmation.",
            parameters: Type.Object({
                accountId: accountIdParam,
                emailId: Type.String({ description: "JMAP draft Email id." }),
                identityId: identityIdParam,
        ...
        },
        {
            ...
            description: "Read one submission or query bounded submission history, including undo and delivery status. This has no side effect.",
            parameters: Type.Object({
                accountId: accountIdParam,
                ...
                undoStatus: Type.Optional(Type.String({ description: "Filter by undoStatus, such as pending or final." })),
        ...
        },
        {
            ...
            description: "Request cancellation only while a submission reports undoStatus=pending, then verify the server reports canceled. This cannot recall delivered mail.",
            parameters: Type.Object({
                accountId: accountIdParam,
            ...
            execute: async (_toolCallId, rawParams) => {
                ...
                return runAuditedJmapTool("jmap_mail_submission_cancel", params, async () => {
                    ...
                    const { account, client } = await resolveClient(optionalString(params.accountId));
                    const submission = await client.cancelSubmission(requiredString(params, "submissionId"));
                    return jsonResult({
                        accountId: account.accountId,
                        externalSideEffect: true,
                        canceled: true,
        ...
        },
        {
            ...
            description: "Move one or more emails exclusively into a destination mailbox selected by id, role, or exact name. This does not issue permanent deletion, but server retention may later purge Trash or Junk.",
            parameters: Type.Object({
                accountId: accountIdParam,
                emailIds: Type.Array(Type.String(), {
                    minItems: 1,
