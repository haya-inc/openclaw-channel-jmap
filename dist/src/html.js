...
export function decodeHtmlEntities(value) {
    return value
        .replace(/&#x([0-9a-f]+);?/gi, (_match, raw) => decodeCodePoint(raw, 16))
        .replace(/&#([0-9]+);?/g, (_match, raw) => decodeCodePoint(raw, 10))
        .replace(/&([a-z]+);/gi, (match, raw) => NAMED_ENTITIES[raw.toLowerCase()] ?? match);
}
...
export function htmlToPlainText(html) {
    return decodeHtmlEntities(stripHiddenHtml(html)
        .replace(/<(br|hr)\b[^>]*\/?>/gi, "\n")
...
export function extractHttpLinks(text, htmlParts = []) {
    const links = new Set();
    const add = (raw) => {
