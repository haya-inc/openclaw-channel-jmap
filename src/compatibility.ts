...
export const JMAP_COMPATIBILITY_SCOPES = ["read", "manage", "send", "full"] as const;

export type JmapServerProfile = keyof typeof JMAP_SERVER_PROFILES;
export type JmapCompatibilityScope = (typeof JMAP_COMPATIBILITY_SCOPES)[number];
export type JmapCompatibilityVerdict =
  | "compatible"
  | "partial"
  | "incompatible"
  | "unverified";
export type JmapCompatibilityCheckStatus = "pass" | "fail" | "skip";
export type JmapCompatibilityFeatureStatus =
  | "verified"
  | "advertised"
  | "unsupported"
  | "unverified";

export type JmapCompatibilityCheck = {
  id: string;
  status: JmapCompatibilityCheckStatus;
  required: boolean;
  evidence: "invoked" | "advertised" | "not-advertised" | "not-run";
  code: string;
  durationMs?: number;
};

export type JmapCompatibilityReport = {
  schemaVersion: 1;
  generatedAt: string;
  serverProfile: JmapServerProfile;
  serverLabel: string;
  scope: JmapCompatibilityScope;
  accountId: string;
  authMode: JmapAuthMode;
  verdict: JmapCompatibilityVerdict;
  advertisedCapabilities: string[];
  checks: JmapCompatibilityCheck[];
  features: {
    receivePolling: JmapCompatibilityFeatureStatus;
    search: JmapCompatibilityFeatureStatus;
    read: JmapCompatibilityFeatureStatus;
    thread: JmapCompatibilityFeatureStatus;
    update: JmapCompatibilityFeatureStatus;
    send: JmapCompatibilityFeatureStatus;
...
function requirementsFor(scope: JmapCompatibilityScope): Set<string> {
  if (scope === "manage") {
...
function hasCapability(state: JmapClientInit, capability: string): boolean {
...
function featureFromCheck(
  checks: JmapCompatibilityCheck[],
  id: string,
  success: JmapCompatibilityFeatureStatus,
): JmapCompatibilityFeatureStatus {
...
export function isJmapServerProfile(value: string): value is JmapServerProfile {
  return Object.hasOwn(JMAP_SERVER_PROFILES, value);
}

export function isJmapCompatibilityScope(value: string): value is JmapCompatibilityScope {
  return (JMAP_COMPATIBILITY_SCOPES as readonly string[]).includes(value);
}
...
export async function runJmapCompatibilityCheck(params: {
  ...
  serverProfile?: JmapServerProfile;
  scope?: JmapCompatibilityScope;
}): Promise<JmapCompatibilityReport> {
  ...
  const addCheck = (
    id: string,
    status: JmapCompatibilityCheckStatus,
    evidence: JmapCompatibilityCheck["evidence"],
    code: string,
    durationMs?: number,
  ) => {
    report.checks.push({
  ...
  addCheck(
    "core-capability",
    coreAvailable ? "pass" : "fail",
    coreAvailable ? "advertised" : "not-advertised",
    coreAvailable ? "capability-advertised" : "capability-missing",
  );
  addCheck(
    "mail-capability",
    mailAvailable ? "pass" : "fail",
    mailAvailable ? "advertised" : "not-advertised",
    mailAvailable ? "capability-advertised" : "capability-missing",
  );
  ...
  addCheck(
    "mailbox-rights",
    hasManageRights ? "pass" : "skip",
    hasManageRights ? "advertised" : "not-run",
    hasManageRights ? "management-rights-advertised" : "rights-not-reported",
  );
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          receivePollingVerified = true;
          addCheck(
            "poll-snapshot-fallback",
            "pass",
            "invoked",
            "recent-query-dedupe-fallback",
  ...
  addCheck(
    "receive-polling",
    receivePollingVerified ? "pass" : "fail",
    receivePollingVerified ? "invoked" : "not-run",
    receivePollingVerified ? "poll-strategy-verified" : "poll-strategy-unavailable",
  );
  ...
  try {
    ...
    addCheck(
      "email-metadata",
      metadata.sampleEmailFound ? "pass" : "skip",
      metadata.sampleEmailFound ? "invoked" : "not-run",
      metadata.sampleEmailFound ? "email-get-succeeded" : "empty-mailbox",
      Date.now() - metadataStartedAt,
    );
    addCheck(
      "thread-get",
      metadata.threadAvailable
        ? metadata.threadGetVerified
          ? "pass"
          : "fail"
        : "skip",
      metadata.threadAvailable ? "invoked" : "not-run",
      metadata.threadAvailable
        ? metadata.threadGetVerified
          ? "method-succeeded"
          : "method-failed"
  ...
  addCheck(
    "submission-capability",
    submissionAvailable ? "pass" : "fail",
    submissionAvailable ? "advertised" : "not-advertised",
    submissionAvailable ? "capability-advertised" : "capability-missing",
  );
  addCheck(
    "identity-get",
    state.identityId && state.identityEmail ? "pass" : submissionAvailable ? "fail" : "skip",
    state.identityId && state.identityEmail ? "invoked" : "not-run",
    state.identityId && state.identityEmail
      ? "identity-available"
      : submissionAvailable
  ...
  addCheck(
    "download-url",
    state.downloadUrl ? "pass" : "fail",
    state.downloadUrl ? "advertised" : "not-advertised",
    state.downloadUrl ? "template-advertised" : "template-missing",
  );
  addCheck(
    "upload-url",
    state.uploadUrl ? "pass" : "fail",
    state.uploadUrl ? "advertised" : "not-advertised",
    state.uploadUrl ? "template-advertised" : "template-missing",
  );
  addCheck(
    "event-source-url",
    state.eventSourceUrl ? "pass" : "fail",
    state.eventSourceUrl ? "advertised" : "not-advertised",
    state.eventSourceUrl ? "template-advertised" : "template-missing",
  );
  ...
  report.features = {
    ...
    read:
      report.checks.find((entry) => entry.id === "email-metadata")?.code === "empty-mailbox"
        ? mailAvailable
          ? "advertised"
          : "unsupported"
    ...
    send:
      submissionAvailable && Boolean(state.identityId && state.identityEmail)
        ? "advertised"
        : "unsupported",
  ...
  const emptyMailboxOnly = requiredChecks.every(
    (entry) =>
      entry.status === "pass" ||
      (entry.id === "email-metadata" && entry.code === "empty-mailbox") ||
      entry.id === "thread-get",
  );
  const missingRequired = requiredChecks.some(
    (entry) =>
      entry.status === "fail" ||
      (entry.status === "skip" &&
        !(entry.id === "email-metadata" && entry.code === "empty-mailbox")),
...
export function compatibilityExitCode(verdict: JmapCompatibilityVerdict): number {
...
export function formatJmapCompatibilityReport(report: JmapCompatibilityReport): string {
  const lines = [
  ...
  for (const check of report.checks) {
    lines.push(
      `  ${check.status.toUpperCase().padEnd(4)} ${check.id}${check.required ? " [required]" : ""} (${check.code})`,
    );
  }
  lines.push(
    "Safety: no mail sent, no mailbox state changed, no message body read, no message identifier printed.",
  );
  if (
    report.scope === "send" ||
    report.scope === "full" ||
